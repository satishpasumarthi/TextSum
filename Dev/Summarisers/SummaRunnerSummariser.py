...
from Dev.DataTools import summarunner_datareader
...
from Dev.Models.SummariserNetClassifier.summariser_net import graph, sents2input, ABSTRACT_DIMENSION,NUM_FEATURES, MAX_SENT_LEN
...
SUMM_SOURCE = BASE_DIR + "/Data/Papers/For_SummaRunner/"

SUMMARY_WRITE_LOC = BASE_DIR + "/Data/Generated_Data/Generated_Summaries/SummaRunnerSummariser/"
...
HIDDEN_SIZE = "2"
#SAVE_PATH = BASE_DIR+"/Trained_Models/"+MODEL_NAME+"/"+HIDDEN_SIZE+"_units/"
SAVE_PATH = '/scratch/user/phan3006/MATH689/SummaRuNNer/runs/1540590378/checkpoints/'
...
class SummariserNetSummariser(Summariser):
    ...
    def summarise(self, filename):
        ...
        with tf.Session() as sess:
            ...
            feed_x = summarunner_datareader.get_input_tensor(SUMM_SOURCE+filename)
            #print(self.prediction_probs,feed_x)

            input_x = graph1.get_operation_by_name("inputs/x_input").outputs[0]
            self.prediction_probs = graph1.get_operation_by_name("score_layer/prediction").outputs[0]
            
            # Predict how good a summary each sentence is using the computation graph
            probs = np.random.random(0)
            for x in feed_x:
                probs = np.append(sess.run(self.prediction_probs, feed_dict={input_x: x.reshape(40,100)}),probs)
